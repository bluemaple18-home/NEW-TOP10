...
if ($null -eq $Credential -and $AllowEnvironmentFallback) {
    ...
    if ($accountValue -and $pinValue) {
        $Credential = [PSCredential]::new($accountValue, (ConvertTo-SecureString $pinValue -AsPlainText -Force))
        [Environment]::SetEnvironmentVariable('YUANTA_ACCOUNT', $null, 'Process')
        [Environment]::SetEnvironmentVariable('YUANTA_PIN', $null, 'Process')
        $accountValue = $null
        $pinValue = $null
    }
}
