...
4. 既有 loop 已啟用且 queue 低於 `TOP10_PM_RESEARCH_MIN_QUEUE_DEPTH=12` 時，會先用 autonomous research manager 產生 active `topic_bank.json`；題庫由 ranking candidate 乘上 `standard`、`risk_guard`、`long_horizon`、`tight_exit` validation profile 組成。題目一進 queue 就從 active bank 移除，做完後只留在 `topic_registry` / `run_history`，不再回到題目庫。補題只產 research-only 候選，不改 ranking/model/publish。
