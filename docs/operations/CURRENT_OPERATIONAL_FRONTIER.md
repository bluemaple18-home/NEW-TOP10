...
Read-only observation、evidence mapping 與已完成 A2/A3 的 closeout reconciliation 可繼續；A4 只是 next frontier，仍為 `BLOCKED / AWAITING_SEPARATE_OWNER_ADMISSION`，A5–A6 維持 `BLOCKED / NOT_STARTED`。Research lane 不得碰 scheduler、publish、production、runtime、config 或 operational data mutation。Issue #10 natural-run observation 與 A3 完全分離，不得由 A3 completion 推論或改寫 #9/#10。任何缺少個別證據的 claim 必須標示 `UNKNOWN` 或 `UNPINNED_RUNTIME_ARTIFACT`，不得用 projection receipt 或狀態文案宣稱 runtime load。
