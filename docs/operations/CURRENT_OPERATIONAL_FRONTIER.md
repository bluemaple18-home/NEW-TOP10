...
Read-only observation、evidence mapping 與已完成 A2 的 closeout reconciliation 可繼續；A3–A6 仍須個別 admission。Research lane 不得碰 scheduler、publish、production、runtime、config、schema 與資料 mutation。Issue #10 natural-run observation 與 A2 acceptance 完全分離，不得由 A2 狀態推論或改寫 #9/#10。任何缺少個別證據的 claim 必須標示 `UNKNOWN` 或 `UNPINNED_RUNTIME_ARTIFACT`，不得用 projection receipt 或狀態文案宣稱 runtime load。
