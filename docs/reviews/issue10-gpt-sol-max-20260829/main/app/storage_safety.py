...
_WARNING_SAMPLE_SCHEDULE_NUMERATOR = 1
_WARNING_SAMPLE_SCHEDULE_DENOMINATOR = 2
DAILY_JOB = "daily"
SOFT_SWAP_WARNING = "SOFT_SWAP_WARNING"
SWAP_EMERGENCY_HARD_STOP = "SWAP_EMERGENCY_HARD_STOP"
...
class JobPolicy:
    ...
    swap_warning_growth_bytes: int | None = None
...
def load_policy(path: Path, job: str) -> tuple[GlobalPolicy, JobPolicy, tuple[RetentionRule, ...]]:
    ...
    if raw.get("swap_warning_growth_bytes") is not None:
        if job != DAILY_JOB:
            raise ValueError(f"{job}.swap_warning_growth_bytes 只允許 daily 設定")
        swap_warning_growth_bytes = _positive_int(
            raw["swap_warning_growth_bytes"],
            f"{job}.swap_warning_growth_bytes",
        )
    job_policy = JobPolicy(
        ...
        swap_warning_growth_bytes=swap_warning_growth_bytes,
    )
    if job_policy.sample_interval_seconds > 300:
        raise ValueError(f"{job}.sample_interval_seconds 不得超過 300")
    if (
        job_policy.swap_warning_growth_bytes is not None
        and job_policy.swap_warning_growth_bytes >= job_policy.max_swap_growth_bytes
    ):
        raise ValueError(f"{job}.swap_warning_growth_bytes 必須小於 max_swap_growth_bytes")
...
def _max_swap_growth_bytes(samples: Sequence[Sample]) -> int | None:
    """以全部 samples 從 baseline 重算 host-wide swap 最大增量。"""

    if not samples or samples[0].swap_bytes is None:
        return None
    observed = [sample.swap_bytes for sample in samples if sample.swap_bytes is not None]
    if not observed:
        return None
    return max(observed) - samples[0].swap_bytes


def _daily_swap_warning_enabled(policy: JobPolicy) -> bool:
    return policy.job == DAILY_JOB and policy.swap_warning_growth_bytes is not None


def runtime_warnings(policy: JobPolicy, samples: Sequence[Sample]) -> tuple[str, ...]:
    warnings: list[str] = []
    swap_delta = _max_swap_growth_bytes(samples)
    if (
        _daily_swap_warning_enabled(policy)
        and swap_delta is not None
        and swap_delta >= int(policy.swap_warning_growth_bytes)
    ):
        warnings.append(SOFT_SWAP_WARNING)
    return tuple(dict.fromkeys(warnings))


def _live_sample_schedule_interval(policy: JobPolicy, samples: Sequence[Sample]) -> float:
    base_interval = (
        policy.sample_interval_seconds
        * _LIVE_SAMPLE_SCHEDULE_NUMERATOR
        / _LIVE_SAMPLE_SCHEDULE_DENOMINATOR
    )
    if SOFT_SWAP_WARNING not in runtime_warnings(policy, samples):
        return base_interval
    return (
        policy.sample_interval_seconds
        * _WARNING_SAMPLE_SCHEDULE_NUMERATOR
        / _WARNING_SAMPLE_SCHEDULE_DENOMINATOR
    )
...
def evaluate_runtime(
    ...
    *,
    ...
    rss_limit_reached = (
        max(observed_rss) >= policy.max_process_tree_rss_bytes
        if observed_rss and _daily_swap_warning_enabled(policy)
        else observed_rss and max(observed_rss) > policy.max_process_tree_rss_bytes
    )
    ...
    if (
        _daily_swap_warning_enabled(policy)
        and swap_delta is not None
        and swap_delta >= policy.max_swap_growth_bytes
    ):
        reasons.append(SWAP_EMERGENCY_HARD_STOP)
    elif (
        not _daily_swap_warning_enabled(policy)
    ...
    return StopDecision(
        ...
        runtime_warnings(policy, samples),
    )
    ...
    *,
    ...
    return {
        ...
        "limits": {
            ...
            "swap_warning_growth_bytes": policy.swap_warning_growth_bytes,
        ...
        "summary": {
            ...
            "peak_swap_growth_bytes": _max_swap_growth_bytes(samples),
        ...
        "warnings": list(runtime_warnings(policy, samples)),
...
def run_guarded_job(
    ...
    *,
    ...
    try:
        ...
        sample_schedule_interval = _live_sample_schedule_interval(policy, samples)
        ...
        def sample_live_child() -> tuple[float, float] | None:
            ...
            nonlocal last_safe_observation_at, next_sample_target
            ...
            active_interval = _live_sample_schedule_interval(policy, samples)
            if active_interval < sample_schedule_interval:
                next_sample_target = min(
                    next_sample_target,
                    last_safe_observation_at + active_interval,
                )
        ...
        def next_sample_action(sample_duration: float) -> str:
            ...
            if next_sample_target <= now:
                missed_intervals = (
                    math.floor(
                        (now - next_sample_target) / active_interval
                    )
                    + 1
                )
                next_sample_target += missed_intervals * active_interval
            sample_hard_deadline = (
        ...
        while process.poll() is None:
            ...
            if deadline_reached:
                ...
                if scheduled_reason is not None:
                    ...
                    terminate_process_group(process, identity=process_group)
                    break
                next_sample_target += _live_sample_schedule_interval(policy, samples)
                if scheduled_observation is not None:
