...
def fixture_job_policy(**overrides: object) -> JobPolicy:
    policy = JobPolicy(
        ...
        swap_warning_growth_bytes=None,
    )
...
class StorageSafetyRegressionTest(unittest.TestCase):
    def test_policy_contract_is_complete_and_verified_jobs_have_evidence(self) -> None:
        ...
        verified_jobs = {"external-review-preflight"}
        ...
        _global_policy, daily_policy, _rules = load_policy(policy_path, "daily")
        self.assertFalse(daily_policy.launch_verified)
        self.assertIn("Issue #10 isolated candidate", daily_policy.verification_basis)
        self.assertIn("0~448,853,443 bytes swap delta", daily_policy.verification_basis)
        self.assertIn("未驗證 >2GiB swap growth", daily_policy.verification_basis)
        self.assertIn("production live 入口", daily_policy.verification_basis)
        self.assertIn("deterministic tests", daily_policy.verification_basis)
        self.assertIn("S2 warning 後 sampling density", daily_policy.verification_basis)
        self.assertIn("Owner", daily_policy.verification_basis)
        self.assertEqual(daily_policy.swap_warning_growth_bytes, 2147483648)
        self.assertEqual(daily_policy.max_swap_growth_bytes, 4294967296)
        preflight_sample = Sample(
            0,
            1,
            1,
            245_107_195_904,
        ...
        production_preflight = evaluate_preflight(
            _global_policy,
            daily_policy,
            preflight_sample,
            validation_only=False,
        )
        validation_preflight = evaluate_preflight(
            _global_policy,
            daily_policy,
            preflight_sample,
            validation_only=True,
        )
        self.assertEqual(production_preflight.reasons, ("POLICY_NOT_LIVE_VERIFIED",))
        self.assertFalse(validation_preflight.triggered)
        ...
        invalid_warning = json.loads(policy_path.read_text(encoding="utf-8"))
        invalid_warning["jobs"]["retrain"]["swap_warning_growth_bytes"] = 2147483648
        with tempfile.TemporaryDirectory(prefix="top10-storage-policy-warning-") as tmp:
            invalid = Path(tmp) / "invalid.json"
            invalid.write_text(json.dumps(invalid_warning), encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "只允許 daily"):
                load_policy(invalid, "retrain")

        invalid_warning = json.loads(policy_path.read_text(encoding="utf-8"))
        invalid_warning["jobs"]["daily"]["swap_warning_growth_bytes"] = 4294967296
        with tempfile.TemporaryDirectory(prefix="top10-storage-policy-warning-") as tmp:
            invalid = Path(tmp) / "invalid.json"
            invalid.write_text(json.dumps(invalid_warning), encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "必須小於"):
                load_policy(invalid, "daily")
    ...
    def test_daily_swap_over_warning_budget_records_warning_without_stop(self) -> None:
        gib = 1024**3
        policy = fixture_job_policy(
            max_process_tree_rss_bytes=4 * gib,
            max_swap_growth_bytes=4 * gib,
            swap_warning_growth_bytes=2 * gib,
        )
        samples = [
            Sample(0, 10, 1, 100_000, 50_000, 100, 0, phase="preflight"),
            Sample(1, 10, 1, 100_000, 50_000, 100, 2 * gib, phase="live"),
            Sample(2, 10, 1, 100_000, 50_000, 100, 2 * gib + 1, phase="final"),
        ]

        decision = evaluate_runtime(fixture_global_policy(), policy, samples)

        self.assertFalse(decision.triggered)
        self.assertEqual(decision.reasons, ())
        self.assertEqual(decision.warnings, ("SOFT_SWAP_WARNING",))

    def test_daily_swap_emergency_cap_stops_and_denies_restart(self) -> None:
        gib = 1024**3
        with tempfile.TemporaryDirectory(prefix="top10-storage-daily-swap-hard-") as tmp:
            root = Path(tmp)
            (root / "output").mkdir()
            policy = fixture_job_policy(
                max_process_tree_rss_bytes=4 * gib,
            ...
            def sampler(pid: int | None) -> Sample:
                ...
                sample_index += 1
                swap = 0 if pid is None else 4 * gib
                return Sample(time.time(), 0, 0, 100_000, 50_000, 1024, swap)
            ...
            self.assertIn("SWAP_EMERGENCY_HARD_STOP", receipt["reasons"])
            self.assertEqual(receipt["warnings"], ["SOFT_SWAP_WARNING"])
            self.assertEqual(receipt["limits"]["swap_warning_growth_bytes"], 2 * gib)
            self.assertEqual(denial["reasons"], receipt["reasons"])
            self.assertIs(denial["automatic_clear_allowed"], False)

    def test_daily_rss_and_swap_rising_still_stops_below_emergency_cap(self) -> None:
        gib = 1024**3
        policy = fixture_job_policy(
            max_process_tree_rss_bytes=4 * gib,
            max_swap_growth_bytes=4 * gib,
            swap_warning_growth_bytes=2 * gib,
            expected_growth_bytes_per_hour=10 * gib,
            spike_window_seconds=999,
            stabilize_after_seconds=999,
        )
        samples = [
            Sample(0, 10, 1, 100_000, 50_000, 100, 0, phase="preflight"),
            Sample(1, 10, 1, 100_000, 50_000, 100, 2 * gib, phase="live"),
            Sample(2, 10, 1, 100_000, 50_000, 200, 2 * gib + 1, phase="live"),
            Sample(3, 10, 1, 100_000, 50_000, 300, 2 * gib + 2, phase="live"),
        ]

        decision = evaluate_runtime(fixture_global_policy(), policy, samples)

        self.assertTrue(decision.triggered)
        self.assertIn("RSS_AND_SWAP_RISING", decision.reasons)
        self.assertNotIn("SWAP_GROWTH_BUDGET_EXCEEDED", decision.reasons)
        self.assertEqual(decision.warnings, ("SOFT_SWAP_WARNING",))

    def test_daily_host_and_rss_caps_remain_hard_stops(self) -> None:
        gib = 1024**3
        global_policy = replace(fixture_global_policy(), runtime_min_free_percent=0.1)
        policy = fixture_job_policy(
            max_process_tree_rss_bytes=4 * gib,
            max_swap_growth_bytes=4 * gib,
            swap_warning_growth_bytes=2 * gib,
        ...
        host_decision = evaluate_runtime(
            global_policy,
            policy,
            [
                Sample(0, 10, 1, 100_000, 50_000, 100, 0, phase="preflight"),
                Sample(1, 10, 1, 100_000, 9_999, 100, 2 * gib, phase="live"),
            ],
        )
        rss_decision = evaluate_runtime(
            fixture_global_policy(),
            policy,
            [
                Sample(0, 10, 1, 100_000, 50_000, 100, 0, phase="preflight"),
                Sample(1, 10, 1, 100_000, 50_000, 4 * gib, 2 * gib, phase="live"),
            ],
        )

        self.assertIn("HOST_RUNTIME_FREE_SPACE_BELOW_THRESHOLD", host_decision.reasons)
        self.assertIn("PROCESS_TREE_RSS_BUDGET_EXCEEDED", rss_decision.reasons)
        self.assertEqual(host_decision.warnings, ("SOFT_SWAP_WARNING",))
        self.assertEqual(rss_decision.warnings, ("SOFT_SWAP_WARNING",))

    def test_legacy_jobs_keep_absolute_swap_budget(self) -> None:
        gib = 1024**3
        policy = fixture_job_policy(
            job="retrain",
            max_swap_growth_bytes=2 * gib,
            swap_warning_growth_bytes=None,
        )
        samples = [
            Sample(0, 10, 1, 100_000, 50_000, 100, 0, phase="preflight"),
            Sample(1, 10, 1, 100_000, 50_000, 100, 2 * gib + 1, phase="live"),
        ]

        decision = evaluate_runtime(fixture_global_policy(), policy, samples)

        self.assertTrue(decision.triggered)
        self.assertIn("SWAP_GROWTH_BUDGET_EXCEEDED", decision.reasons)
        self.assertEqual(decision.warnings, ())

    def test_daily_swap_stoploss_rollback_restores_absolute_2gib_gate(self) -> None:
        gib = 1024**3
        policy = fixture_job_policy(
            max_swap_growth_bytes=2 * gib,
            swap_warning_growth_bytes=None,
    ...
    def test_daily_swap_warning_tightens_live_sampling_interval(self) -> None:
        gib = 1024**3
        with tempfile.TemporaryDirectory(prefix="top10-storage-warning-cadence-") as tmp:
            root = Path(tmp)
            (root / "output").mkdir()
            clock = FakeMonotonicClock()
            waits: list[float] = []
            ...
            def sampler(pid: int | None) -> Sample:
                return Sample(
                    ...
                    host_free_bytes=50_000,
                    rss_bytes=1024,
                    swap_bytes=0 if pid is None else 2 * gib,
                )
            ...
            self.assertEqual(receipt["warnings"], ["SOFT_SWAP_WARNING"])
            self.assertEqual(receipt["summary"]["peak_swap_growth_bytes"], 2 * gib)
            self.assertEqual(receipt["summary"]["swap_bytes_delta"], 0)

    def test_daily_swap_warning_density_leaves_normal_cadence_unchanged(self) -> None:
        gib = 1024**3
        with tempfile.TemporaryDirectory(prefix="top10-storage-warning-normal-cadence-") as tmp:
            root = Path(tmp)
            (root / "output").mkdir()
            clock = FakeMonotonicClock()
            waits: list[float] = []
    ...
    def test_daily_swap_warning_density_late_sampler_fails_without_busy_loop(self) -> None:
        gib = 1024**3
        with tempfile.TemporaryDirectory(prefix="top10-storage-warning-late-sampler-") as tmp:
            root = Path(tmp)
            (root / "output").mkdir()
            clock = FakeMonotonicClock()
            waits: list[float] = []
            ...
            def wait_for_process(
                ...
                waits.append(timeout)
                raise AssertionError("warning density schedule-overrun 前不應進入 waiter")
            ...
            self.assertEqual(receipt["reasons"], ["LIVE_SAMPLE_SCHEDULE_OVERRUN"], context)
            self.assertEqual(receipt["warnings"], ["SOFT_SWAP_WARNING"], context)
            ...
            self.assertEqual(waits, [], context)
            self.assertTrue(all(timeout > 0 for timeout in waits), context)
            self.assertIsNotNone(target_pid)
            with self.assertRaises(ProcessLookupError):
                os.kill(int(target_pid), 0)

    def test_daily_swap_warning_density_preserves_hard_runtime_precedence(self) -> None:
        gib = 1024**3
        with tempfile.TemporaryDirectory(prefix="top10-storage-warning-runtime-") as tmp:
            root = Path(tmp)
            (root / "output").mkdir()
            clock = FakeMonotonicClock()
            waits: list[float] = []
            ...
            self.assertEqual(receipt["warnings"], ["SOFT_SWAP_WARNING"], context)
            self.assertEqual(live_completions, [1.0], context)
            self.assertEqual(waits, [4.5], context)
            self.assertTrue(all(timeout > 0 for timeout in waits), context)
            self.assertAlmostEqual(clock(), 5.5, msg=context)
            self.assertEqual(denial["reasons"], receipt["reasons"], context)
            self.assertFalse(denial["automatic_clear_allowed"])
            self.assertIsNotNone(target_pid)
            with self.assertRaises(ProcessLookupError):
