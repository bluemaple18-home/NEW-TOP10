...
`docs/operations/top10-storage-policy.json` 目前將 `daily.launch_verified=false`。Issue #10
daily swap stop-loss candidate 已整合進 main，但仍是 `PRE_PRODUCTION_NO_GO`：local deterministic
tests 與 isolated readiness evidence 不等於已授權 load、enable、kickstart、手動執行 live
launchd、清除 restart marker 或 send。
...
`daily.launch_verified=false` 代表 daily 仍會被 storage policy pre-child fail-closed 擋下；
實際載入、啟用、kickstart 或手動執行 live launchd 仍是外部控制面動作，必須另有
operator 授權。對仍為 `launch_verified=false` 的 job：
