...
| 14 | PM Research Harness Bot | 只消費 PM 核准的 TOP10_STOCK 研究卡，驅動下一輪受控研究；queue 低水位時先補 research-only 候選題 | PM decision state、PM review cards、manager state | approved work queue、research cards、PM harness status、topic discovery artifact | launchd 明確啟用研究；未核准不產新卡、不送 Discord |
...
- PM Research Harness Bot 只在 PM 明確核准 TOP10_STOCK review card 後，才把核准項目轉成研究 queue 並驅動既有 research runner；正式 launchd 會明確帶 enable env 讓它持續跑研究。若既有 loop 已啟用且 queue 低於 12，會先整理題庫，必要時把高分 rejected 題補成 revisit 候選，避免輪空。
...
- `TOP10_PM_RESEARCH_MIN_QUEUE_DEPTH=12` 與 `TOP10_PM_RESEARCH_DISCOVERY_MAX_TOPICS=30` 控制低水位補題；補題只更新 autonomous research manager queue、revisit action 與 discovery artifact，不改 ranking/model/publish。
