...
- 2026-09-02：R15 attribution cycle peak `1,635,909,632 bytes`，最大 contributor 是 `build_weekend_universe_inventory.py` 的單一 PID `1,612,349,440 bytes`；這推翻「主峰已定位在 strategy matrix」的先前推測。惟 historical exact-regime fixture 仍產生空 `topic_runs`，整體維持 `NO-GO / REPRESENTATIVE_WORKLOAD_EMPTY`，不得把本輪低於 2 GiB 宣稱為完整 capacity PASS。
- 2026-09-02：R15 empty root cause 已在本機 RED 證實：fixture candidate 被 `is_baseline_like()` 正確排除，導致 `generated=[] / no_executable_ranking_template=1`。fixture 改用既有 exact-date、非 baseline-like 的 `vwap_narrow_only_balanced` candidate 後 fresh-manager selection GREEN；受影響回歸 `99 passed, 31 subtests passed`。仍須新 external cycle 才能證明完整 topic capacity。
- 2026-09-02：R16 第一輪在 176.03 秒由 hard ceiling 停止；process-tree peak `2,236,661,760 bytes`，其中 weekend inventory 單 PID `2,202,353,664 bytes`，memory pressure 全程 `1`、swap 減少、unknown writes `[]`，未跑第二輪。historical fixture 因 inventory 先停而尚未執行。
