...
## Post-card technical access probe

2026-07-20 在 owner 明確要求確認「是否實際拿得到資料」後，另做一次最小唯讀單日探針。TWSE 官方 T86 response endpoint 對交易日 `20260717` 回傳 `stat=OK`、19 欄、1,337 列，且本次 request 未使用登入或 token。

此 probe 更新的是技術事實：`TECHNICAL_ACCESS=GO`、盤後每日排程在技術上可行。它不回溯改變原卡的研究範圍，也不單獨證明官方 rate、retention、redistribution 或 production authorization；source-governance decision 仍為 `KEEP_BLOCKED`。完整證據記錄於 `docs/evidence/TSKG-MFO-SRC-01/verification.md` §7。
