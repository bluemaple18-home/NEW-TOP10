...
model: gpt-5.6-sol
reasoning: high
model_reason: candidate改動current／historical universe scope與fail-closed verifier，錯誤會重新開啟live circuit或虛報分類進度，需獨立高風險契約審查
...
# FOG-MAP-BURN-DOWN-UNIVERSE-ALIGNMENT-01 Independent Review
...
- 先讀主卡、candidate diff、RED／GREEN evidence，再獨立執行必要驗證。
- 不得修改production code或測試；發現問題只寫finding與verdict。

## Review questions

1. Producer是否以current canonical expanded universe作`full_universe_total`，同時保留
   historical rollup實際source scope，而非偽造新增classification？
2. 合法stale-smaller partial是否滿足current total、classified subset、pending delta與
   category conservation，且map verifier可通過？
3. over-classified、negative／missing pending、category sum mismatch、missing／
   mismatched source scope是否仍fail closed？
4. same-scope full classification是否仍為100%，base／expanded／executed progress與
   HTML contract是否未退化？
5. candidate是否只改主卡allowlist，無topic supply、dimension、queue／retry、
   ranking／model／promotion／closed registry變更？
6. Executor所列兩個full-suite failure是否確為candidate外環境／時區問題，而非本卡
   regression？
...
- Candidate changed files：7；production files僅
  `app/research/fog_map_domain.py`、`scripts/verify_research_fog_map.py`。
...
- 只以P0／P1阻擋：資料不守恆、錯誤放行、current universe失真、candidate導致handoff
  failure、越界改動或無法重現核心GREEN。
...
產生`.work/FOG-MAP-BURN-DOWN-UNIVERSE-ALIGNMENT-01/review/review_receipt.md`，內容含：

- fixed base／candidate／review commit
- checks與逐項結果
- findings（含severity、證據與是否blocking）
- allowlist audit與remaining risks
- 唯一verdict：`REVIEW_GO`或`REVIEW_NO_GO`
