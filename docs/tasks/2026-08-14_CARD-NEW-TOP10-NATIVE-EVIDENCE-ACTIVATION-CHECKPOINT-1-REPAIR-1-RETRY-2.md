---
id: CARD-NEW-TOP10-NATIVE-EVIDENCE-ACTIVATION-CHECKPOINT-1-REPAIR-1-RETRY-2
...
# Native Evidence Activation Repair 1 Retry 2

## Replacement／Retry 原因

原 Repair task 連續三次因已回收／非 writable worktree 與 `apply_patch` 路由不一致而無法修改；沒有本 finding 的 diff 或 commit。使用者已明確核准 replacement。`RETRY-1` 在 create 前因 card role identity mismatch fail-closed，沒有形成 create request或thread；本卡延續同一 chain、role、cycle與finding，不重置 Repair 額度。
...
需要其他 functional path 時，先回主線附證據並停止。不得修改其他 `docs/tasks/**`、production ranking、model、signals、strategy config、promotion、launchd、daily scheduler或 Adaptive Queue。
...
- 單一 repair candidate commit；不得 push、merge、deploy、live或 production write。
- 完成後送回既有 Reviewer task targeted re-review。
- 衝突、需要 allowlist 外檔案、identity/root 不明或同一 blocker再現即停止。

## Rollback

捨棄 replacement Repair candidate；保留 `19cfc78...`、Reviewer evidence與正式 artifacts不變。
