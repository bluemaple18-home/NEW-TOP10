#!/usr/bin/env python3
"""驗證 regime conditional hybrid report。"""
...
SCHEMA_VERSION = "regime-conditional-hybrid-report-verification.v1"
REPORT_SCHEMA = "regime-conditional-hybrid-report.v1"
REQUIRED_SIDES = {"production", "candidate_all", "hybrid_big_bull"}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="verify regime conditional hybrid report")
    parser.add_argument("--artifact", required=True)
    parser.add_argument("--output", default="artifacts/model_experiments/regime_conditional_hybrid_report_verification_latest.json")
...
def build_payload(path: Path) -> dict[str, Any]:
    ...
    checks = [
        ...
        {"name": "required_sides_present", "ok": REQUIRED_SIDES <= sides, "value": sorted(str(side) for side in sides)},
        {"name": "rows_complete", "ok": len(rows) >= len(capital_levels) * len(REQUIRED_SIDES), "value": len(rows)},
