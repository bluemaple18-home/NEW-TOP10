#!/usr/bin/env python3
"""驗證 chip warning-only shadow report。"""

from __future__ import annotations

import argparse
import json
from pathlib import Path


PROJECT_ROOT = Path(__file__).resolve().parents[1]
SCHEMA_VERSION = "chip-warning-shadow-report.v1"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="verify chip warning-only shadow report")
    parser.add_argument(
        "--artifact",
        default="artifacts/model_experiments/chip_warning_shadow_report_2026-06-06.json",
    )
    return parser.parse_args()


def resolve_path(value: str | Path) -> Path:
    path = Path(value).expanduser()
    return path if path.is_absolute() else PROJECT_ROOT / path


def main() -> int:
    args = parse_args()
    artifact = resolve_path(args.artifact)
    errors: list[str] = []
    if not artifact.exists():
        errors.append(f"artifact missing: {artifact}")
        print(json.dumps({"status": "FAILED", "errors": errors}, ensure_ascii=False))
        return 1

    payload = json.loads(artifact.read_text(encoding="utf-8"))
    contract = payload.get("contract") or {}
    decision = payload.get("decision") or {}
    replay = payload.get("replay") or {}

    if payload.get("schema_version") != SCHEMA_VERSION:
        errors.append("schema_version mismatch")
    if payload.get("status") not in {"OK", "BLOCKED"}:
        errors.append("status must be OK or BLOCKED")
    for key in ("research_only", "warning_only", "does_not_send_push"):
        if contract.get(key) is not True:
            errors.append(f"contract.{key} must be true")
    for key in ("changes_model", "changes_production_ranking", "changes_risk_adjusted_score"):
        if contract.get(key) is not False:
            errors.append(f"contract.{key} must be false")
    if decision.get("production_status") != "BLOCKED":
        errors.append("production must remain blocked")
    if payload.get("status") == "OK":
        if replay.get("observation_count", 0) <= 0:
            errors.append("OK report must include replay observations")
        if decision.get("status") not in {"SHADOW_REPLAY_MONITOR_ONLY", "NOT_STABLE_ENOUGH_FOR_WARNING_CHANNEL"}:
            errors.append("unexpected OK decision status")
    if len(payload.get("candidate_rules") or []) < 4:
        errors.append("candidate rules too thin")

    status = "OK" if not errors else "FAILED"
    print(json.dumps({"status": status, "artifact": str(artifact), "errors": errors}, ensure_ascii=False))
    return 0 if not errors else 1


if __name__ == "__main__":
    raise SystemExit(main())
