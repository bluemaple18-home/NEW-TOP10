#!/usr/bin/env python3
"""驗證 baseline harness small-window replay。"""
...
VERIFY_SCHEMA_VERSION = "baseline-harness-small-window-replay-verification.v1"
ARTIFACT_SCHEMA_VERSION = "baseline-harness-small-window-replay.v1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="verify baseline harness small-window replay")
    parser.add_argument("--date", default=date.today().isoformat())
    parser.add_argument("--artifact", default=None)
    parser.add_argument(
        "--output",
        default="artifacts/weekend_training/baseline_harness_small_window_replay_verification_latest.json",
    )
...
def artifact_path(run_date: str, override: str | None) -> Path:
    if override:
        ...
        return path
    return PROJECT_ROOT / "artifacts" / "weekend_training" / f"baseline_harness_small_window_replay_{run_date}.json"
...
def build_payload(run_date: str, artifact: Path) -> dict[str, Any]:
    ...
    checks = [
        ...
        {"name": "small_window_status_ok", "ok": payload.get("small_window_status") == "OK", "value": payload.get("small_window_status")},
        ...
        {"name": "ranking_file_count_gte_10", "ok": isinstance(payload.get("ranking_file_count"), int) and payload.get("ranking_file_count") >= 10, "value": payload.get("ranking_file_count")},
        {"name": "actual_replay_count_gte_10", "ok": isinstance(payload.get("actual_replay_count"), int) and payload.get("actual_replay_count") >= 10, "value": payload.get("actual_replay_count")},
        {
            "name": "runner_bound_to_small_window",
        ...
        {"name": "production_impact_no_change", "ok": payload.get("production_impact") == PRODUCTION_IMPACT, "value": payload.get("production_impact")},
        {
            ...
            and contract.get("small_window_only") is True
    ...
    return {
        ...
        "verification_summary": {
            ...
            "small_window_status": payload.get("small_window_status"),
...
def main() -> int:
    ...
    print(f"BASELINE_HARNESS_SMALL_WINDOW_REPLAY_VERIFICATION_{payload['status']} output={repo_path(output)}")
