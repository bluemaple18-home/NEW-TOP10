#!/usr/bin/env python3
"""執行 baseline harness medium-window replay。

只使用 WEEKEND-TRAINING-19 review 指定的 100D 視窗，產生 research-only
medium-window baseline 並跑 bounded replay；不建立 artifacts/backtest/production，
不跑 202,176 格，不改模型與正式 ranking。
"""
...
SCHEMA_VERSION = "baseline-harness-medium-window-replay.v1"
TASK_ID = "WEEKEND-TRAINING-20"
REVIEW_ARTIFACT = "artifacts/weekend_training/baseline_harness_medium_window_review_2026-06-18.json"
REVIEW_VERIFICATION = "artifacts/weekend_training/baseline_harness_medium_window_review_verification_latest.json"
DEFAULT_START_DATE = "2025-12-24"
DEFAULT_END_DATE = "2026-05-15"
TARGET_BASELINE_PATH = "artifacts/backtest/production_baseline_harness_medium_window"
STAGING_OUTPUT_TEMPLATE = "artifacts/weekend_training/staging/production_baseline_harness_medium_window_{date}"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="run baseline harness medium-window replay")
    parser.add_argument("--date", default=date.today().isoformat())
    parser.add_argument("--review-artifact", default=REVIEW_ARTIFACT)
    parser.add_argument("--review-verification", default=REVIEW_VERIFICATION)
...
def output_paths(run_date: str, override: str | None) -> tuple[Path, Path]:
    if override:
        ...
        json_path = PROJECT_ROOT / "artifacts" / "weekend_training" / f"baseline_harness_medium_window_replay_{run_date}.json"
...
def review_window(review_artifact: dict[str, Any], review_verification: dict[str, Any]) -> tuple[str, str, str, int]:
    if review_artifact.get("medium_window_review_status") != "OK":
        raise RuntimeError("medium-window review artifact is not OK")
    if review_verification.get("status") != "OK":
        raise RuntimeError("medium-window review verification is not OK")
    if review_artifact.get("can_run_medium_window") is not True:
        raise RuntimeError("medium-window review does not allow replay")
    label = str(review_artifact.get("recommended_medium_window") or "")
    start_date = str(review_artifact.get("recommended_start_date") or "")
    end_date = str(review_artifact.get("recommended_end_date") or "")
    trading_day_count = int(review_artifact.get("recommended_trading_day_count") or 0)
    if label != "100D" or start_date != DEFAULT_START_DATE or end_date != DEFAULT_END_DATE:
        raise RuntimeError(f"review window must be 100D {DEFAULT_START_DATE}~{DEFAULT_END_DATE}")
    if trading_day_count < MIN_RANKING_COUNT:
        raise RuntimeError(f"review trading day count must be >= {MIN_RANKING_COUNT}: {trading_day_count}")
    return label, start_date, end_date, trading_day_count


def build_harness_args(args: argparse.Namespace, staging_dir: Path, start_date: str, end_date: str) -> argparse.Namespace:
...
def copy_medium_window_baseline(staging_manifest: dict[str, Any], staging_manifest_path: Path, target_dir: Path) -> list[dict[str, Any]]:
...
def materialize_manifest(
    ...
    review_verification_path: Path,
) -> dict[str, Any]:
    manifest = {
        "schema_version": "research-only-production-baseline-medium-window-manifest.v1",
        ...
        "source_review_artifact": repo_path(review_artifact_path),
        "source_review_verification": repo_path(review_verification_path),
        ...
        "contract": {
            "research_only": True,
            "medium_window_only": True,
            "review_window_only": True,
            ...
            "medium_window_ok_is_not_full_replay_unlock": True,
        },
    }
...
def build_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    review_artifact_path = resolve_path(args.review_artifact)
    review_verification_path = resolve_path(args.review_verification)
    staging_dir = resolve_path(STAGING_OUTPUT_TEMPLATE.format(date=run_date))
    target_dir = resolve_path(args.target_dir)
    assert review_artifact_path is not None and review_verification_path is not None and staging_dir is not None and target_dir is not None

    review_artifact = read_json(review_artifact_path)
    review_verification = read_json(review_verification_path)
    window_label, start_date, end_date, review_trading_day_count = review_window(review_artifact, review_verification)
    harness_args = build_harness_args(args, staging_dir, start_date, end_date)
    ...
    if harness_status != "OK":
        raise RuntimeError(f"harness medium-window staging blocked: {harness_smoke.get('blocker_reasons')}")
    if FORBIDDEN_PRODUCTION_PATH.exists():
    ...
    copied_files = copy_medium_window_baseline(staging_manifest, staging_manifest_path, target_dir)
    materialized_manifest = materialize_manifest(
        run_date,
        target_dir,
    ...
    if contains_forbidden_text(review_artifact, review_verification, harness_smoke, staging_manifest, materialized_manifest):
    ...
    return {
        ...
        "source_review_artifact": repo_path(review_artifact_path),
        "source_review_verification": repo_path(review_verification_path),
        "recommended_medium_window": window_label,
        "review_trading_day_count": review_trading_day_count,
        "window": {"start": start_date, "end": end_date, "dates": file_dates},
        ...
        "next_action": "WEEKEND-TRAINING-21_baseline_harness_unlock_policy_review" if status == "OK" else "修 medium-window harness/replay blocker，不跑全量。",
    }
...
def render_markdown(payload: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# Baseline Harness Medium-Window Replay",
            "",
            f"- medium_window_status: `{payload.get('medium_window_status')}`",
            ...
            f"- start_date: `{(payload.get('window') or {}).get('start')}`",
            f"- end_date: `{(payload.get('window') or {}).get('end')}`",
...
def main() -> int:
    ...
    try:
        ...
        print(f"BASELINE_HARNESS_MEDIUM_WINDOW_REPLAY_FAILED {exc}", file=sys.stderr)
        return 1
    json_path, md_path = output_paths(str(args.date), args.output)
    write_json(json_path, payload)
    write_text(md_path, render_markdown(payload))
    print(f"BASELINE_HARNESS_MEDIUM_WINDOW_REPLAY_{payload['medium_window_status']} output={repo_path(json_path)}")
    return 0 if payload["medium_window_status"] == "OK" else 1
