...
ATTRIBUTION_VERIFICATION_SCHEMA = "training-candidate-risk-attribution-verification.v1"
ATTRIBUTION_REPORT_SCHEMA = "training-candidate-risk-attribution.v1"
RISK_CONTROL_VERIFICATION_SCHEMA = "training-candidate-risk-control-report-verification.v1"
RISK_CONTROL_REPORT_SCHEMA = "training-candidate-risk-control-report.v1"
DEFAULT_OUTPUTS = {
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="verify training candidate risk reports")
...
def build_attribution_payload(path: Path) -> dict[str, Any]:
    ...
    checks = [
        {"name": "schema", "ok": payload.get("schema_version") == ATTRIBUTION_REPORT_SCHEMA, "value": payload.get("schema_version")},
    ...
    return {
        "schema_version": ATTRIBUTION_VERIFICATION_SCHEMA,
...
def build_risk_control_payload(path: Path) -> dict[str, Any]:
    payload = read_json(path)
    contract = payload.get("contract") if isinstance(payload.get("contract"), dict) else {}
    ...
    checks = [
        {"name": "schema", "ok": payload.get("schema_version") == RISK_CONTROL_REPORT_SCHEMA, "value": payload.get("schema_version")},
        {"name": "status_ok", "ok": payload.get("status") == "OK", "value": payload.get("status")},
        {"name": "research_only", "ok": contract.get("research_only") is True, "value": contract},
        {"name": "model_changes_false", "ok": contract.get("model_changes") is False, "value": contract.get("model_changes")},
        {
    ...
    failed = [check for check in checks if not check["ok"]]
    return {
        "schema_version": RISK_CONTROL_VERIFICATION_SCHEMA,
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "status": "OK" if not failed else "FAILED",
        "artifact": repo_path(path),
        "summary": {
...
def build_payload(profile: str, path: Path) -> dict[str, Any]:
    if profile == "attribution":
        return build_attribution_payload(path)
    if profile == "risk_control":
        return build_risk_control_payload(path)
