#!/usr/bin/env python3
"""驗證 odd-lot HIGH_CHOPPY 推薦日降曝險報告。"""
...
SCHEMA_VERSION = "odd-lot-regime-throttle-report-verification.v1"
REPORT_SCHEMA = "odd-lot-regime-throttle-report.v1"
REQUIRED_VARIANTS = {"baseline", "hc45", "hc55", "hc65"}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="verify odd-lot regime throttle report")
    parser.add_argument("--artifact", required=True)
    parser.add_argument("--output", default="artifacts/model_experiments/odd_lot_regime_throttle_report_verification_latest.json")
...
def build_payload(path: Path) -> dict[str, Any]:
    ...
    checks = [
        ...
        {"name": "required_variants_present", "ok": REQUIRED_VARIANTS <= variants, "value": sorted(str(item) for item in variants)},
