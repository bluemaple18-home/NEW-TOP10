#!/usr/bin/env python3
"""驗證 shadow historical evidence report 的邊界與基本證據。"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="verify shadow historical evidence report")
    parser.add_argument("--artifact", default=None)
    parser.add_argument("--date", default=None)
    parser.add_argument("--expected-model-sha256", default=EXPECTED_MODEL_SHA256)
    parser.add_argument("--output", default="artifacts/model_experiments/shadow_historical_evidence_verification_latest.json")
...
def latest_artifact() -> Path | None:
    files = sorted(OUTPUT_DIR.glob("shadow_historical_evidence_*.json"))
...
def artifact_path(args: argparse.Namespace) -> Path | None:
    ...
    if args.date:
        return OUTPUT_DIR / f"shadow_historical_evidence_{args.date}.json"
...
def main() -> int:
    ...
    checks = {
        ...
        "schema": payload.get("schema_version") == "shadow-historical-evidence-report.v1",
        "report_only": contract.get("report_only") is True,
        "uses_historical_backtest": contract.get("uses_historical_backtest") is True,
        "production_not_ready": contract.get("production_ready") is False,
        ...
        "waiting_only_false": summary.get("waiting_only") is False,
        "historical_backtest_available": summary.get("historical_backtest_available") is True,
        "historical_ranking_count_sufficient": int(summary.get("historical_ranking_count") or 0) >= 500,
    ...
    output.write_text(
        json.dumps(
            {
                "schema_version": "shadow-historical-evidence-verification.v1",
