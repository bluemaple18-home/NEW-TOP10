...
def test_selection_uses_only_d_row_and_canonical_d1_h20_calendar() -> None:
    ...
    observations, exclusions = feasibility._entry_rows(
    ...
    same_observations, same_exclusions = feasibility._entry_rows(
        ranking_dates=ranking_dates,
        regime_rows=rows,
        trade_dates=calendar,
        scenarios=_scenario(calendar),
...
def test_global_split_purges_both_boundaries_with_twenty_trade_day_embargo() -> None:
    ...
    split = feasibility.build_global_split(observations, calendar)
...
def test_validator_rejects_false_go_and_outcome_metric() -> None:
    payload = {
        ...
        "split": {"schema_version": "entry-cohort-calendar-split.v1", "status": "INSUFFICIENT_GLOBAL_DATES", "roles": {}, "boundaries": [], "metric": "sharpe"},
    }
